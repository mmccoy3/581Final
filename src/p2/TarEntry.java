...
class		TarEntry
	...
	public TarEntry( String headerName ){
		this.initialize();
		this.header.nameTarHeader( headerName );
		}
	...
	public String getUserName(){
		return this.header.getUserName();
	}
	...
	public String getGroupName() {
		return this.header.getGroupName();
	}
	...
	public void adjustEntryName( byte[] outbuf, String newName ){
		int offset = 0;
		offset = TarHeader.getNameBytes( new StringBuffer( newName ),outbuf, offset, TarHeader.NAMELENGTH );
	}
	...
	public boolean isDirectory(){
		...
		if ( this.header != null && ( this.header.getLinkFlag() == TarHeader.LF_DIR || this.header.getName().endsWith( "/" )) )
			{
			return true;
			}
	 ...
	 *
	 * @param header The TarHeader to fill in.
	 * @param file The file from which to get the header information.
	 */
	public void getFileTarHeader( TarHeader header, File file )throws InvalidHeaderException{
		this.file = file;

		String fileName = file.getPath();
		String osname = System.getProperty("os.name");
			String Win32Prefix = "windows";
			if (osname != null && osname.toLowerCase().startsWith(Win32Prefix) && fileName.length() > 2) {
				char firstCharacter = fileName.charAt(0);
				char secondCharacter = fileName.charAt(1);
				if (secondCharacter == ':' && ((Character.isLowerCase(firstCharacter) || Character.isUpperCase(firstCharacter)))) {
					fileName = fileName.substring(2);
					}
			}
		fileName = fileName.replace(File.separatorChar, '/');
		for (; fileName.startsWith("/");) {
			fileName = fileName.substring(1);
		}
		header.initializeHeader(file, fileName);
