...
public class TarHeader
	...
	public static long parseOctal( byte[] header, int offset, int length )throws InvalidHeaderException {
		...
		for ( int i = offset ; i < end ; ++i ){
			if ( header[i] == 0 || header[i] == (byte) ' ' || header[i] == '0' ) 
				break;	
	...
	/**
	 * Helper function for the getOctalBytes.
	 * @param buf buffer
	 * @param idx index 
	 * @param value octal bytes
	 * @param offset The offset into the buffer from which to parse.
	 * @return new buffer
	 */
	protected static byte[] fillArray(byte[] buf, int idx, long value, int offset) {
	...
	protected void initializeToZero() {
